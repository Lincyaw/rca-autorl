...
- centring 三种都能配：RLOO / GRPO 走 `actor.reward_norm`，ReMax 走 `econfig.remax`（组内第 0 个 sample 以 temperature 0 跑，作 baseline）。temperature 由 `gconfig.temperature` 经 `RCA_TEMPERATURE` 传给 harness 的 `rca-sampling` 路由，不在别处设
- spec §4 fork 未实现：要从某一轮恢复 session，dsh SDK 没有 resume；runtime 是 pinned 的二进制 wheel
